...
# Load data
ecervi <- 
  read.table("https://raw.githubusercontent.com/LivingLandscapes/Course_EcologicalModeling/data/DeerEcervi.txt", 
             header = TRUE)
...
models <- forms %>%
  mutate(RHS = paste(fe, re, sep = "+"),
         model = paste(LHS, RHS))
fits <- by_row(models, ~glmer(as.formula(.x$model),
                           data=ecervi, 
                           family=binomial))
```
