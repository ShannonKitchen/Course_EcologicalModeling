...
Invasive plants cause immense ecological and economic damage in many places. Understanding 

<p>
!['Cogongrass' by Nancy Loewenstein and John McGuire.](https://github.com/LivingLandscapes/Course_EcologicalModeling/raw/master/images/Cogongrass_Alabama.jpg){width=600px}
<p>

### Data description

The data you will be using is from the U.S. Forest Service's "Forest Inventory and Analysis" dataset. 
...
**On your own, familiarize yourself with the data.** Below, you can see that we're deadling with >100k rows and a few potential grouping variables.
...
fia <-
  ...
  mutate(cover = cover + 0.001, # for log link
         ) %>%
  mutate(across(c(Year, Latitude, Longitude),
                ~ (.x - mean(.x)) / sd(.x),
