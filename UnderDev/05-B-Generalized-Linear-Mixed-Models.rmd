...
# Get residuals 
residM0a <- broom.mixed::augment(M0a)
# # Get quantile residuals
# residM0a$qr <- qresiduals(M0a) # FYI: error prevents knitting, but kept here
# for completeness.
simulationOutput <- simulateResiduals(fittedModel = M0a, plot = T)
...
```{r convergedModels}
# first rename .out
fits <- rename(fits, fit = .out)
fits$converged <- !map_lgl(fits$fit, ~length(.x@optinfo$conv$lme4)>0)
...
ggplot(pp, aes(x=csLength, y=.mu, col=fSex, group=Farm:fSex)) +
  geom_line(alpha=0.5) + 
